summarize_sc_benchmark <- function(results,
  ...
  out <- lapply(groups, function(x) {
    ...
    data.frame(
      ...
      mean_robust_donor_weight = safe_mean(x$mean_robust_donor_weight),
      min_robust_donor_weight = safe_mean(x$min_robust_donor_weight),
      max_donor_penalty_fit = safe_mean(x$max_donor_penalty_fit),
